    ...
    if request.user.is_authenticated():
        ...
        if request.method == "POST":
            ...
            if citationid == 'NotSet':
                citation = Citations(title=request.POST['title'], publisher=request.POST['publisher'],
                                     publicationyear=int(request.POST['publicationyear']),
                                     citationlink=request.POST['citationlink'])
        ...
        return render(request, 'publications3.html',
                      {'Authorformset': Authorformset, 'Citationpropertyformset': Citationpropertyformset,
...
def web_map(request):
    if request.user.is_authenticated():
        ...
        legend_ref = [
            dict(feature_type="Excavation", icon="fa-exclamation-triangle", color="darkred",
            ...
            dict(feature_type="Transect", icon="fa-line-chart", color="cadetblue",
...
def TimeSeriesGraphingShort(request, feature_action='NotSet', samplingfeature='NotSet', dataset='NotSet',
                            resultidu='NotSet', startdate='NotSet', enddate='NotSet'):  # ,startdate='',enddate=''
    authenticated = True
    if not request.user.is_authenticated():
        # return HttpResponseRedirect('../')
        authenticated = False
...
def scatter_plot(request):
    ...
    pr = Results.objects.filter(resultid__in=prv).filter(
        ~Q(featureactionid__samplingfeatureid__sampling_feature_type="Landscape classification")) \
    ...
    return TemplateResponse(request, 'soilsscatterplot.html', {'prefixpath': CUSTOM_TEMPLATE_PATH,
                                                               ...
                                                               'graphType': graphType,
                                                               'yAxis': yAxis, 'xAxis': xAxis, 'xdata': xdata,
                                                               'ydata': ydata, 'ylocation': ylocation,
                                                               'xlocation': xlocation,}, )
...
def exportspreadsheet(request, resultValuesSeries, profileResult=True):
    ...
    resultValuesHeaders = resultValuesSeries.filter(
        ~Q(resultid__resultid__featureactionid__samplingfeatureid__sampling_feature_type="Landscape classification")). \
    ...
    if profileResult:
        resultValuesSeries = resultValuesSeries.filter(~Q(
            resultid__resultid__featureactionid__samplingfeatureid__sampling_feature_type="Landscape classification")). \
            filter(~Q(resultid__resultid__featureactionid__samplingfeatureid__sampling_feature_type="Field area")). \
...
def graph_data(request):
    ...
    relatedFeatureList = Relatedfeatures.objects.filter(samplingfeatureid__in=samplefeatid).order_by(
        'relatedfeatureid').distinct('relatedfeatureid')  #
    ...
    if 'export_data' in request.POST:
        ...
        # raise ValidationError(relatedFeatureList)
        return TemplateResponse(request, 'chartVariableAndFeature.html',
                                {'prefixpath': CUSTOM_TEMPLATE_PATH, 'variableList': variableList,
                                 ...
                                 'chartID': chartID, 'chart': chart, 'series': series, 'title2': title2,
                                 'graphType': graphType, 'yAxis': yAxis, 'name_of_units': name_of_units,
                                 'relatedFeatureList': relatedFeatureList,
                                 'SelectedRelatedFeature': selected_relatedfeatid,}, )
